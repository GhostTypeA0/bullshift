...
async function loadPendingGroup() {
    ...
    let groupMembers = [currentUser.username]; //MAX OF 8 MEMBERS IS BEING ENFORCED

    friends.forEach(f => {
        const friendName =
            f.user1 === currentUser.username ? f.user2 : f.user1;
        ...
        button.addEventListener("click", () => {
            ...
            if (button.classList.contains("selected")) {
                ...
                const groupIndex =
                    groupMembers.indexOf(friendName);
                if (groupIndex !== -1) {
                    groupMembers.splice(groupIndex, 1);
                }
            }
        });
    ...
    createBtn.onclick = () => {
        ...
        const names = Array.from(selected).map(b => b.textContent);

        if (names.length < 2) {
            alert("Please select at least 2 of your friends to make a group.");
            return;
        }
...
//THIS IS THE LOGIC FOR ACTUALLY DOING STUFF WITH THE GROUPS THAT WERE MADE
    async function groupChatCreate(groupMembers) {
        alert(groupMembers); //TEMPORARY


    try {
        const res = await fetch(
            "/api/groupchats/create",
            {
                method: "POST",
                headers: {
